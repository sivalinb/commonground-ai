...
const sensitiveRules = [
  ...
  [
    'student identifier',
    /\b(?:student|juvenile|school)\s*(?:id|number|no\.?|#)\s*[:#-]?\s*[A-Z0-9-]{4,}\b/i,
  ],
] as const;
...
export function validateCitationUsage(
  ...
  const invalidIds = claims
    .flatMap((claim) => claim.citation_ids)
    .filter((id) => !allowed.has(id));
  const uncitedClaims = claims.filter(
    (claim) => claim.citation_ids.length === 0,
  ).length;
  const selectedIds = [
    ...new Set(claims.flatMap((claim) => claim.citation_ids)),
  ];
  return {
...
export function safeAbstention(
  ...
  return {
    ...
    options: [
      {
        text: 'Pause the automated analysis and preserve participant choice.',
        citation_ids: citationIds,
      },
      {
        text: 'Consult approved local policy and a trained practitioner.',
        citation_ids: citationIds,
      },
      {
        text: 'Document the unanswered question for corpus and supervisor review.',
        citation_ids: citationIds,
      },
    ],
    safeguards: [
