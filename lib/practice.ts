...
const stateSchema = z.object({
  ...
  participant: z
    .custom<z.infer<typeof participantSchema>>()
    .nullable()
    .default(null),
  ...
  evaluator: z
    .custom<z.infer<typeof evaluatorSchema>>()
    .nullable()
    .default(null),
...
const runtimeRegistry = new Map<
  string,
  PracticeRuntime & { tracer: MetadataTracer }
>();
...
const pineconeResponseSchema = z.object({
  matches: z
    .array(z.object({ id: z.string(), score: z.number().default(0) }))
    .default([]),
});
...
const practiceGraph = new StateGraph(stateSchema)
  .addNode('evidence_agent', async (state: PracticeState, config) => {
    ...
    return runtime.tracer.stage(
      'evidence_agent',
      'Evidence agent',
      'Fireworks embedding · Pinecone + BM25 · RRF · reranker',
      async () => {
        const query = `${state.scenario}\nLearner response: ${state.learnerResponse}`;
        const embedding = embeddingsResponseSchema.parse(
          await fireworks(
            runtime,
            '/embeddings',
            {
        ...
        const allowed =
          state.jurisdiction === 'colorado'
            ...
            : ['United States'];
        const pinecone = await fetchWithPolicy(
          `https://${runtime.pineconeHost}/query`,
          {
            ...
            body: JSON.stringify({
              ...
              filter: { jurisdiction: { $in: allowed } },
            }),
          },
        ...
        if (!pinecone.ok)
          throw new Error(
            `Pinecone practice retrieval returned ${pinecone.status}`,
          );
        ...
        const lexical = bm25Search(
          query,
          approvedKnowledge.filter((item) =>
            allowed.includes(item.jurisdiction),
          ),
        ).slice(0, 8);
        ...
        if (!candidates.length)
          return { stoppedReason: 'No approved evidence was retrieved.' };
        const reranked = rerankResponseSchema.parse(
          await fireworks(
            runtime,
            '/rerank',
            {
        ...
        return isEvidenceSufficient(evidence)
          ? { evidence }
          : {
              evidence,
              stoppedReason:
                'Evidence confidence was below the practice-lab threshold.',
            };
      },
    );
  })
  .addNode('participant_agent', async (state: PracticeState, config) => {
    ...
    return runtime.tracer.stage(
      'participant_agent',
      'Fictional participant agent',
      'Role-play response with explicit consent signal',
      async () => ({
        participant: await structuredCall(
          runtime,
          runtime.fastModel,
          participantSchema,
          'fictional_participant_v1',
          participantJson,
  ...
  .addNode('coach_agent', async (state: PracticeState, config) => {
    ...
    return runtime.tracer.stage(
      'coach_agent',
      'Facilitator coach agent',
      'Skill feedback grounded in retrieved public evidence',
      async () => {
        const coach = await structuredCall(
          runtime,
          runtime.chatModel,
          coachSchema,
          'facilitator_coach_v1',
          coachJson,
        ...
        return {
          coach: {
            ...coach,
            evidence_ids: coach.evidence_ids.filter((id) => ids.includes(id)),
          },
        };
      },
    );
      ...
      async () => {
        ...
        if (runtime.mistralKey) {
          const reviewed = await mistralStructured({
            ...
            schemaName: 'victim_services_review_v1',
            jsonSchema: advocateJson,
            system,
            user,
          });
          return { advocate: reviewed.data };
        }
        return {
          advocate: await structuredCall(
            runtime,
            runtime.chatModel,
            advocateSchema,
            'victim_services_review_v1',
            advocateJson,
            system,
            user,
          ),
  ...
  .addNode('evaluator_agent', async (state: PracticeState, config) => {
    ...
    return runtime.tracer.stage(
      ...
      'Structured rubric and next practice goal',
      async () => ({
        evaluator: await structuredCall(
          runtime,
          runtime.chatModel,
          evaluatorSchema,
          'practice_evaluator_v1',
          evaluatorJson,
  ...
  .addConditionalEdges(
    'evidence_agent',
    (state) => (state.stoppedReason ? 'end' : 'participant'),
    { end: END, participant: 'participant_agent' },
  )
...
export async function executePractice(
  ...
  await tracer
    .start(input.traceId, input.scenario.length + input.learnerResponse.length)
    .catch(() => undefined);
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        final.stoppedReason ||
          'The practice agents did not produce a complete result.',
      );
    }
    const scores = {
    ...
    const result: PracticeResult = {
      ...
      modelRoles: [
        {
          role: 'Fictional participant',
          model:
            input.runtime.fastModel.split('/').at(-1) ||
            input.runtime.fastModel,
        },
        {
          role: 'Facilitator coach',
          model:
            input.runtime.chatModel.split('/').at(-1) ||
            input.runtime.chatModel,
        },
        {
          role: 'Victim-services reviewer',
          model: input.runtime.mistralKey
            ? `Mistral · ${input.runtime.mistralModel}`
            : input.runtime.chatModel.split('/').at(-1) ||
              input.runtime.chatModel,
        },
        {
