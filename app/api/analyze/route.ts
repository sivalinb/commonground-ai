...
function failureClass(error: unknown) {
  if (error instanceof WorkflowExecutionError) return `${error.stage}_${error.reason}`;
  const name = error && typeof error === 'object' && 'name' in error ? String(error.name) : '';
  if (error instanceof ZodError || name.includes('Zod')) return 'provider_response_validation';
...
export async function POST(request: Request) {
  ...
  if (!request.headers.get('content-type')?.includes('application/json')) {
    return secureJson({ error: 'Content-Type must be application/json.', traceId }, 415);
  }
  if (!(await consumeRateLimit(request, 'analysis', 20, 10 * 60 * 1000))) {
    return secureJson({ error: 'Demo request limit reached. Please try again later.', traceId }, 429);
  }

  try {
    const parsed = analyzeRequestSchema.safeParse(await request.json());
    if (!parsed.success) return secureJson({ error: 'Use a fictional scenario between 20 and 3,000 characters.', traceId }, 400);
    ...
    if (!turnstile.verified) return secureJson({ error: 'Human verification is required before using the live AI demo.', traceId }, 403);
    ...
    if (!fireworksKey || !pineconeKey || !pineconeHost || process.env.LIVE_AI_ENABLED !== 'true') {
      return secureJson({ error: 'The live AI adapter is not configured.', traceId }, 503);
    }
    ...
    if (result.awaitingApproval) await savePendingApproval(result).catch(() => undefined);
    return secureJson({ ...result, turnstileConfigured: turnstile.configured });
  } catch (error) {
    const classified = failureClass(error);
    console.error(traceId, classified, error instanceof Error ? error.message : 'Unknown live analysis error');
    return secureJson({
      error: 'The live analysis could not complete. No action was taken.',
      failureClass: classified,
      failureStage: error instanceof WorkflowExecutionError ? error.stage : undefined,
      traceId,
    }, 502);
  }
}
